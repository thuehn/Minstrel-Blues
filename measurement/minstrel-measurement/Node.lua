...
function Node:set_channel_htmode ( phy, channel, htmode )
    local dev = self:find_wifi_device ( phy )
    if ( dev ~= nil ) then
        return dev:set_channel_htmode ( channel, htmode, self.proc_version )
    end
    return nil
end
...
function Node:get_tcpdump_online ( phy, fname, ms, sz )
    local dev = self:find_wifi_device ( phy )
    if ( dev ~= nil ) then
        return dev:get_tcpdump_online ( fname, ms, sz )
    end
    return nil
end
